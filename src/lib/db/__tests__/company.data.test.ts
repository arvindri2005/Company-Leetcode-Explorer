...
jest.mock("@/app/actions/admin.actions", () => ({
  triggerCompaniesRevalidation: mockTriggerRevalidation,
}));
...
const mockCompanies = [
  {
    ...
    statsLastUpdatedAt: new MockTimestamp(),
  },
  {
    id: "2",
    name: "Company B",
  ...
  },
  {
    ...
    name: "Company C",
    slug: "company-c",
    normalizedName: "company c",
  },
];

const mockFirestoreDocs = (data: any[]) =>
  data.map((item) => ({ id: item.id, data: () => item, exists: () => true }));
...
describe("Company Data", () => {
  ...
  describe("getCompaniesWithTotalCount", () => {
    it("should fetch page 2 correctly", async () => {
      ...
      mockGetDocs.mockResolvedValueOnce({
        docs: mockFirestoreDocs(mockCompanies.slice(0, 2)),
      });
      mockGetDocs.mockResolvedValueOnce({
        docs: mockFirestoreDocs([mockCompanies[2]]),
      });
      const result = await companyData.getCompaniesWithTotalCount({
        page: 2,
        pageSize: 2,
      });
    ...
    it("should handle errors", async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        "Error in getCompaniesWithTotalCount:",
        expect.any(Error),
      );
    });
  });
  ...
  describe("getCompanyById", () => {
    ...
    it("should handle errors", async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        "Error fetching company by ID 1:",
        expect.any(Error),
      );
    });
  });
  ...
  describe("getCompanyBySlug", () => {
    ...
    it("should handle errors", async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        "Error fetching company by slug slug:",
        expect.any(Error),
      );
    });
  });
  ...
  describe("getAllCompanySlugs", () => {
    it("should handle errors", async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        "Error fetching all company slugs:",
        expect.any(Error),
      );
    });
  });
  ...
  describe("addCompanyToDb", () => {
    it("should add a company successfully", async () => {
      ...
      const result = await companyData.addCompanyToDb({
        name: "New Company",
        logo: "logo.png",
        description: "",
        website: "",
    ...
    it("should return error if company exists", async () => {
      mockGetDocs.mockResolvedValue({
        docs: mockFirestoreDocs([mockCompanies[0]]),
        empty: false,
      });
      const result = await companyData.addCompanyToDb({
        name: "Company A",
    ...
    it("should handle add errors", async () => {
      ...
      await companyData.addCompanyToDb({
        name: "New",
        description: "",
        website: "",
        logo: "",
    ...
    it("should handle revalidation failure", async () => {
      mockGetDocs.mockResolvedValue({ docs: [], empty: true });
      mockAddDoc.mockResolvedValue({ id: "new-id" });
      mockTriggerRevalidation.mockRejectedValue(
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        "Failed to revalidate companies page:",
        expect.any(Error),
      );
    });
  });
  ...
  describe("Cache Logic", () => {
    ...
    it("getAllCompanySlugs should cache results", async () => {
      mockGetDocs.mockResolvedValue({
        docs: mockFirestoreDocs(mockCompanies),
        empty: false,
      });
      await companyData.getAllCompanySlugs();
      await companyData.getAllCompanySlugs();
