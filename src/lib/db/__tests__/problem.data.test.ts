...
describe('problem.data', () => {
  ...
  describe('getProblemsByCompanyFromDb', () => {
    it('should filter by difficulty', async () => {
        (getCompanyById as jest.Mock).mockResolvedValue({ id: '1', name: 'Company A', slug: 'company-a' });
      ...
      const result = await getProblemsByCompanyFromDb('1', { difficultyFilter: 'Easy' });
      expect(result.problems).toHaveLength(1);
      expect(result.problems[0].difficulty).toBe('Easy');
    });

    it('should handle errors', async () => {
        (getCompanyById as jest.Mock).mockRejectedValue(new Error('Firestore error'));
  ...
  describe('addProblemToDb', () => {
    it('should add a new problem', async () => {
      ...
      const result = await addProblemToDb('1', { title: 'New Problem', normalizedTitle: 'new problem' });
    ...
    it('should update an existing problem', async () => {
      ...
      const result = await addProblemToDb('1', { title: 'Existing Problem', normalizedTitle: 'existing problem' });
  ...
  describe('getAllProblemCompanyAndProblemSlugs', () => {
    it('should return all slugs', async () => {
        ...
        mockGetDocs.mockResolvedValue({
            docs: [
              { ...createMockDoc({ title: 'Problem 1', slug: 'problem-1' }), ref: { parent: { parent: { id: '1' } } } },
            ],
          });
