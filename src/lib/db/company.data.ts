...
const getCachedFirestoreCompanies = unstable_cache(
  async (currentSearchTerm?: string) => {
    ...
    return querySnapshot.docs.map(docSnap => ({ id: docSnap.id, slug: docSnap.data().slug || slugify(docSnap.data().name), ...docSnap.data() } as Company));
  },
...
export const getCompanies = async ({ page = 1, pageSize = 9, searchTerm }: GetCompaniesParams = {}): Promise<PaginatedCompaniesResponse> => {
  try {
    ...
    if (searchTerm && searchTerm.trim() !== '') {
      ...
      const searchTermPassedToCache = !!(searchTerm && searchTerm.trim() !== '');

      filteredCompanies = filteredCompanies.filter(company =>
        (company.description && company.description.toLowerCase().includes(lowercasedSearchTerm)) ||
        (!searchTermPassedToCache && company.normalizedName && company.normalizedName.includes(lowercasedSearchTerm))
      );
    }
    ...
    // console.error("Error fetching companies: ", error); // Kept for debugging during dev
...
const getCachedCompanyById = unstable_cache(
    async (companyId: string) => {
      ...
      if (companySnap.exists()) {
        ...
        return { id: companySnap.id, slug: data.slug || slugify(data.name), ...data } as Company;
      }
      return undefined;
    },
    ['company-by-id-cache-key-v2'],
    {
      tags: async (companyId: string) => {
        ...
        return [`company-detail-${companyId}`, slug ? `company-slug-${slug}` : 'company-slug-unknown'];
      },
      revalidate: 3600
    }
  );
...
export const getCompanyById = async (id: string): Promise<Company | undefined> => {
  try { return await getCachedCompanyById(id); }
...
const getCachedCompanyBySlug = unstable_cache(
    async (companySlug: string) => {
      ...
      if (!querySnapshot.empty) {
        ...
        return { id: companyDoc.id, slug: data.slug || slugify(data.name), ...data } as Company;
      }
      return undefined;
    },
    ['company-by-slug-cache-key-v2'],
    {
      tags: async (companySlug: string) => {
        const companyQuery = query(collection(db, 'companies'), where('slug', '==', companySlug), limit(1));
        const companySnap = await getDocs(companyQuery);
        if (!companySnap.empty) {
            const companyDoc = companySnap.docs[0];
            return [`company-slug-${companySlug}`, `company-detail-${companyDoc.id}`];
        }
        return [`company-slug-${companySlug}`, 'company-detail-unknown'];
      },
      revalidate: 3600
    }
  );
...
export const getCompanyBySlug = async (slug: string): Promise<Company | undefined> => {
  try { return await getCachedCompanyBySlug(slug); }
  catch (error) { console.error(`Error fetching company by slug ${slug}: `, error); return undefined; }
};
...
const getCachedAllCompanySlugs = unstable_cache(
    async () => {
      ...
      return companiesSnapshot.docs.map(docSnap => docSnap.data().slug as string).filter(Boolean);
    },
...
export const addCompanyToDb = async (companyData: Omit<Company, 'id' | 'slug'>): Promise<{ id: string | null; error?: string; alreadyExists?: boolean }> => {
  try {
    const companySlug = slugify(companyData.name);
    const normalizedName = companyData.name.toLowerCase();

    const slugQuery = query(collection(db, 'companies'), where('slug', '==', companySlug), limit(1));
    const slugSnapshot = await getDocs(slugQuery);
    if (!slugSnapshot.empty) {
      console.warn(`Company with slug "${companySlug}" already exists (ID: ${slugSnapshot.docs[0].id}). Not adding duplicate.`);
      return { id: slugSnapshot.docs[0].id, error: `Company with name "${companyData.name}" (slug: ${companySlug}) already exists.`, alreadyExists: true };
    }
