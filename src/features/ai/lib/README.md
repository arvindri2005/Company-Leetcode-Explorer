...
This directory (`src/lib/ai`) contains the application's AI logic, powered by [Genkit](https://firebase.google.com/docs/genkit) and Google Gemini models.
...
pnpm genkit:dev
```
This runs `src/lib/ai/dev.ts` and opens the developer tool at `http://localhost:4000`.

### 2. Registering New Flows
If you add a new flow file, **you must import it in `src/lib/ai/dev.ts`** for it to appear in the Genkit UI.
...
- **Model**: Configured in `src/lib/ai/genkit.ts`. Currently defaults to `googleai/gemini-flash-lite-latest`.
