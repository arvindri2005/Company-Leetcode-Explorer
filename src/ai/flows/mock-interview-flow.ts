...
const MockInterviewInputSchema = z.object({
  ...
  problemDescription: z
    .string()
    .describe(
      "A concise description of the coding problem, including its core requirements and constraints. This might be derived from the problem link or a summary.",
    ),
  problemTags: z
    .array(z.string())
    .describe(
      "Tags associated with the problem, indicating relevant topics or techniques.",
    ),
  conversationHistory: z
    ...
    .describe(
      'The history of the conversation so far. Alternating user and model messages. "model" is the AI interviewer.',
    ),
  currentUserMessage: z
    .string()
    .describe("The user's latest message, question, code snippet, or attempt."),
  educationHistory: z
    .array(EducationExperienceSchema)
    .optional()
    .describe(
      "The candidate's educational background. Can be used by the AI to tailor questions or hints if relevant.",
    ),
  workHistory: z
    .array(WorkExperienceSchema)
    .optional()
    .describe(
      "The candidate's work experience. Can be used by the AI to tailor questions or hints if relevant.",
    ),
});
...
const FeedbackSchema = z
  .object({
    solutionAssessment: z
      .string()
      .optional()
      .describe(
        "Overall assessment of the user's solution if they submitted one. E.g., Correct, Almost Correct (with details), Incorrect approach. Be concise but clear.",
      ),
    correctnessDetails: z
      .string()
      .optional()
      .describe(
        "Specific details on correctness, bugs, or edge cases missed. Only provide if there are noteworthy points. Be constructive.",
      ),
    timeComplexity: z
      .string()
      .optional()
      .describe(
        "Analysis of the time complexity of the user's solution (e.g., O(n), O(n log n)). Provide if identifiable from code or clear approach.",
      ),
    spaceComplexity: z
      .string()
      .optional()
      .describe(
        "Analysis of the space complexity of the user's solution (e.g., O(1), O(n)). Provide if identifiable.",
      ),
    alternativeApproaches: z
      .array(z.string())
      .optional()
      .describe(
        "Brief suggestions for 1-2 alternative or more optimal approaches, if applicable and distinct from the user's method.",
      ),
    codeQualitySuggestions: z
      .string()
      .optional()
      .describe(
        "Brief suggestions for improving code clarity, style, or best practices (e.g., variable naming, modularity). Only if significant.",
      ),
  })
  .describe(
    "Structured feedback on the user's solution attempt. Provided when the AI deems it a review phase.",
  );
...
const MockInterviewOutputSchema = z.object({
  interviewerResponse: z
    .string()
    .describe(
      "The AI interviewer's conversational response to the user. This should be helpful, guiding, and avoid giving direct solutions unless the user is completely stuck and has asked for a more direct hint, or if they are submitting a final solution for review.",
    ),
  ...
  suggestedFollowUps: z
    ...
    .optional()
    .describe(
      "A list of 1-2 concise follow-up questions or points for the user to consider based on their current message. These are for the user to think about or for the AI to potentially use in a later turn. Generate these if the user has proposed an approach, shared code, or made a significant statement.",
    ),
});
...
const mockInterviewFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    if (!output || !output.interviewerResponse) {
      throw new Error(
        "AI did not return a valid output for the interview turn. Input was: " +
