            ...
            reader.onload = (event) => {
                try {
                    ...
                    if (!arrayBuffer) {
                        reject(
                            new Error(
                                `Could not read file data from ${file.name}.`
                            )
                        );
                        return;
                    }
                    ...
                    if (!firstSheetName) {
                        reject(
                            new Error(
                                `The file ${file.name} does not contain any sheets.`
                            )
                        );
                        return;
                    }
                    ...
                    if (!worksheet) {
                        reject(
                            new Error(
                                `Could not read the first sheet from ${file.name}.`
                            )
                        );
                        return;
                    }
                    ...
                    if (!headerRowJson || headerRowJson.length === 0) {
                        reject(
                            new Error(
                                `Could not read headers from ${file.name}.`
                            )
                        );
                        return;
                    }
                    const headerRow = headerRowJson[0] as string[];
                    const actualHeaders = headerRow.map((h) =>
                        String(h).trim()
                    );
                    if (!actualHeaders.includes("Name")) {
                        reject(
                            new Error(
                                `File ${file.name} is missing required header 'Name'.`
                            )
                        );
                        return;
                    }
                    ...
                    const jsonData =
                        XLSX.utils.sheet_to_json<RawExcelCompanyDataForClient>(
                            worksheet,
                            { defval: "" }
                        );
                    resolve(jsonData);
        ...
        try {
            ...
            if (allCompanies.length === 0) {
                toast({
                    title: "No Data Found",
                    description:
                        "The selected files contain no valid company data.",
                    variant: "destructive",
                });
                setIsProcessing(false);
                return;
            ...
            toast({
                ...
                description:
                    "An unexpected error occurred during the bulk upload process.",
                variant: "destructive",
            });
        } finally {
            setIsProcessing(false);
