    ...
    try {
        ...
        for (const companyInfo of companyIdsAndNames) {
            try {
                ...
                const newDifficultyCounts: Required<
        ...
        const message = `Successfully updated stats for ${updatedCount} companies. ${
            errorList.length > 0 ? `${errorList.length} companies failed.` : ""
        }`;
        console.log(message);
        return {
            success: true,
