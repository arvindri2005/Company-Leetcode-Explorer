...
export async function syncUserProfile(
  ...
  Logger.warn(
    "syncUserProfile Server Action is deprecated. Use userService.syncUserProfile on the client side.",
  );
  return await userService.syncUserProfile(
    userData.uid,
    userData.email,
...
export async function toggleBookmarkProblemAction(
  ...
  try {
    ...
    const errorMessage = handleServerActionError(
      ...
      { userId, problemId, companySlug, problemSlug },
    );
    return {
      success: false,
      error: errorMessage,
