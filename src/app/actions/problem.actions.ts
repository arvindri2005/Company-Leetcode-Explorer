...
interface BulkAddDetailedResult {
  rowIndex: number;
  title: string;
...
export async function bulkAddProblems(
  ...
  updatedCount: number;
  errorCount: number;
  detailedResults: BulkAddDetailedResult[];
}> {
  let added = 0,
    updated = 0,
    errors = 0;
  ...
  const { getCompanies: getAllCompaniesFromDbInternal } = await import(
    "@/lib/data"
  );
  const companiesData = await getAllCompaniesFromDbInternal({
    pageSize: 100000,
  });
  ...
  for (let i = 0; i < problemsFromExcel.length; i++) {
    ...
    if (!companyInfo || !companyInfo.id || !companyInfo.slug) {
      errors++;
      detailedResults.push({
        rowIndex: i,
        title,
    ...
    const diffLower = String(raw.difficulty || "")
      .trim()
      .toLowerCase();
    ...
    if (!difficulty) {
      ...
      detailedResults.push({
        ...
        title,
        status: "error",
        message: `Invalid or missing difficulty: "${raw.difficulty}". Must be Easy, Medium, or Hard.`,
      });
      continue;
    }
    ...
    if (!link.startsWith("http")) {
      ...
      detailedResults.push({
        ...
        title,
        status: "error",
        message: "Invalid problem link. Must start with http:// or https://.",
      });
      continue;
    }
    try {
      ...
      detailedResults.push({
        ...
        title,
        status: "error",
        message: `Malformed problem link: ${link}`,
      });
      continue;
    }
    ...
    const lastAskedPeriod = String(
      raw.lastAskedPeriod || "",
    ).trim() as LastAskedPeriod;
    if (!validLastAskedPeriods.includes(lastAskedPeriod)) {
      errors++;
      detailedResults.push({
        rowIndex: i,
        title,
        status: "error",
        message: `Invalid Last Asked Period: "${lastAskedPeriod}". Valid options: ${validLastAskedPeriods.join(", ")}.`,
      });
      continue;
    }

    const result = await addProblemToDb(companyId, {
      title,
      difficulty,
      link,
      tags,
    ...
    if (result.id) {
      if (result.updated) {
        ...
        detailedResults.push({
          ...
          title,
          status: "updated",
          message: "Updated existing problem.",
        });
      } else {
        added++;
        detailedResults.push({
      ...
      detailedResults.push({
        ...
        title,
        status: "error",
        message: result.error || "Database error occurred.",
      });
    }
  }
  ...
  if (added > 0 || updated > 0) {
    ...
    affectedCompanySlugs.forEach((slug) =>
    ...
    affectedCompanyIds.forEach((id) =>
      revalidateTag(`problems-for-company-${id}`),
    );

    ["/", "/submit-problem", "/bulk-add-problems", "/companies"].forEach((p) =>
      revalidatePath(p),
    );
