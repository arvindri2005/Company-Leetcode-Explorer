...
export default function ProfilePage() {
    ...
    const [isLoadingWorkExperience, setIsLoadingWorkExperience] =
        useState(false);
    const [isWorkDialogOpen, setIsWorkDialogOpen] = useState(false);

    const [isEditingDisplayName, setIsEditingDisplayName] = useState(false);
    const [isSubmittingDisplayName, setIsSubmittingDisplayName] =
        useState(false);
    ...
    const fetchBookmarkedData = async () => {
        if (user?.uid) {
            ...
            const bookmarkInfosResult =
                await getUsersBookmarkedProblemsInfoAction(user.uid);
    ...
    const fetchStatusData = async () => {
        if (user?.uid) {
            ...
            const statusResult = await getAllUserProblemStatusesAction(
                user.uid
            );
            if (
    ...
    const stats = {
        solved: Object.values(problemStatuses).filter(
            (p) => p?.status === "solved"
        ).length,
                            ...
                            <Card className="bg-card border border-border rounded-xl shadow-sm">
                                ...
                                <CardContent>
                                    <ProfileProblemList
                                        ...
                                        listType="bookmarks"
                                        onBookmarkChanged={
                                            handleProblemBookmarkChangeOnProfile
                                        }
                                        onProblemStatusChange={
                                            handleProblemStatusChangeOnProfile
                                        }
                                    />
                                </CardContent>
                            </Card>
