...
export async function POST(request: NextRequest) {
    try {
        ...
        if (pageSize && (typeof pageSize !== "number" || pageSize < 1 || pageSize > 50)) {
            return NextResponse.json({ error: "Invalid pageSize. Must be between 1 and 50" }, { status: 400 });
        }
        if (searchTerm && typeof searchTerm !== "string") {
            return NextResponse.json({ error: "Invalid searchTerm format" }, { status: 400 });
        }
        if (!cursor) {
            return NextResponse.json({ error: "Cursor is required for pagination" }, { status: 400 });
        }

        const result = await loadMoreCompanies(cursor, pageSize, searchTerm?.trim());
        return NextResponse.json(result);
    } catch (error) {
        console.error("Error in companies API route:", error);
        return NextResponse.json({ error: "Internal server error", companies: [], hasMore: false }, { status: 500 });
    }
}
...
export async function GET(request: NextRequest) {
    ...
    if (!cursor) {
        return NextResponse.json({ error: "Cursor parameter is required" }, { status: 400 });
    }
