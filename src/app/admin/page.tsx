...
function UpdateStatsButton() {
  ...
  const handleUpdateStats = async () => {
    ...
    toast({
      title: "Processing...",
      description:
        "Updating company problem statistics. This may take a moment.",
    });
  ...
  return (
    <Card className="hover:shadow-md transition-shadow">
      <CardHeader>
        ...
        <CardDescription>
          Recalculate and update problem statistics (difficulty, recency, common
          tags) for all companies. This can be resource-intensive and should be
      ...
      <CardContent className="space-y-3">
        ...
        {resultMessage && (
          <div
            className={`mt-3 p-3 rounded-md text-sm ${errorDetails ? "bg-red-50 border-red-200 text-red-700" : "bg-green-50 border-green-200 text-green-700"}`}
          >
            <p>{resultMessage}</p>
            {errorDetails && errorDetails.length > 0 && (
              <div className="mt-2">
                ...
                <ul className="list-disc list-inside text-xs">
                  {errorDetails.slice(0, 5).map(
                    (
                      err,
                      index, // Show first 5 errors
                    ) => (
                      <li key={index}>
                        {err.companyName} (ID: {err.companyId}): {err.error}
                      </li>
                    ),
                  )}
                  {errorDetails.length > 5 && (
                    <li>
                      ...and {errorDetails.length - 5} more errors. Check server
                      logs.
                    </li>
                  )}
                </ul>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card className="hover:shadow-md transition-shadow">
          <CardHeader>
            ...
            <CardDescription>
              Upload an Excel (.xlsx) or CSV (.csv) file to add multiple
              companies at once.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Button asChild>
              <Link href="/admin/bulk-add-companies">
                Manage Company Uploads
              </Link>
            </Button>
          </CardContent>
        </Card>
        ...
        <Card className="hover:shadow-md transition-shadow">
          ...
          <CardContent>
            <Button asChild>
              <Link href="/admin/bulk-add-problems">
                Manage Problem Uploads
              </Link>
            </Button>
          </CardContent>
        </Card>
