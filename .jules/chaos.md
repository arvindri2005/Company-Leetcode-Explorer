...
### 2024-05-25 - [Root Layout] Entropy: Header Navigation Failure
*   **Failure Mode**: The global `Header` component relies on complex contexts (`AuthContext`, `NavigationRegistry`) and hooks. If any of these fail or throw an error during rendering (e.g., navigation item generation logic bug), the error bubbles up to the `RootLayout`. Since Next.js `error.tsx` only catches errors in *nested* segments, a Header crash results in a catastrophic application-wide failure (generic 500 page or white screen), rendering the entire application unusable.
*   **Resilience Strategy**: Wrapped the `Header` component in a granular `ErrorBoundary` within `src/app/layout.tsx` and implemented a `HeaderErrorFallback`. This ensures that if navigation fails, the core page content remains accessible, preventing a "White Screen of Death" and allowing users to continue using the application (albeit with limited navigation).
